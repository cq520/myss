...
Libtest(){
	...
	if [ "$libAddr" == "$GIT" ];then
		libAddr='https://raw.githubusercontent.com/cq520/myss/master/libsodium-1.0.17.tar.gz'
	else
		libAddr='https://raw.githubusercontent.com/cq520/myss/master/libsodium-1.0.17.tar.gz'
	fi
	rm -f ping.pl		
...
install_node(){
	...
	echo "# https://github.com/Anankke/shadowsocks                                                    "
	...
	read -p "检测节点延迟建议关闭: " SPEEDTEST 
	read -p "检测云安全建议关闭: " CLOUDSAFE 
	...
	wget -N -P  /etc/ --no-check-certificate  https://raw.githubusercontent.com/cq520/myss/master/supervisord.conf
	supervisord
	#iptables
	iptables -F
	iptables -X  
